...
require_once("include/imagens_produtos_variables.php");
...
$page_layouts["imagens_produtos_detailspreview"] = $layout;
...
if($mastertable == "produtos")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("id_produto") . "=" . make_db_value("id_produto",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	//	id_produto - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("id_produto", $data, $keylink);
			$row["id_produto_value"] = $value;
			$format = $pSet->getViewFormat("id_produto");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
	...
	//	imagem - File-based Image
			...
			$format = $pSet->getViewFormat("imagem");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
	...
	//	destaque - Checkbox
			...
			$format = $pSet->getViewFormat("destaque");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("destaque")))
				$class = ' rnr-field-number';
			$row["destaque_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("imagens_produtos", "detailspreview"));
