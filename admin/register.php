...
$layout = new TLayout("register2", "CoralOrange", "MobileOrange");
$layout->version = 2;
...
$layout = new TLayout("register_success2", "CoralOrange", "MobileOrange");
$layout->version = 2;
$layout->blocks["top"] = array();
$layout->containers["register"] = array();
$layout->container_properties["register"] = array(  );
...
//	processing senha
	$control_senha = $pageObject->getControl("senha", $id);
	$control_senha->readWebValue($values, $blobfields, NULL, NULL, $filename_values);


	$strUsername = $values["email"];
	$strPassword = $values["senha"];
...
//	check if entered username already exists
	...
	{
		$pageObject->jsSettings['tableSettings'][$strTableName]['msg_userError'] = "Nome do usuário não pode estar vazio";
		$allow_registration = false;
	}	
	else
	{
		$strSQL = "select count(*) from ". $pageObject->connection->addTableWrappers( "usuarios" ) 
			. " where " . 
		...
		if( $data[0] > 0 )
		{
			$pageObject->jsSettings['tableSettings'][$strTableName]['msg_userError'] = "Nome do Usuário"." <i>".$strUsername."</i> "."já existe. Escolha outro nome";
			$allow_registration = false;
		}
	}
	...
	if($retval)
	{
		$retval = DoInsertRecord("usuarios", $values, $blobfields, $pageObject);
	}
