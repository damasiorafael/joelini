...
$layout = new TLayout("remind2", "CityAvenue", "MobileAvenue");
$layout->version = 2;
...
if (@$_POST["btnSubmit"] == "Remind")
{
	...
	{	
		...
		if($tosearch)
		{
			...
			$strSQL = $selectClause." from ".$pageObject->connection->addTableWrappers("usuarios")." where ".$sWhere;
			...
			if($data)
			{
					...
					if(!$cipherer->isFieldEncrypted($cPasswordField))
				{
					...
					$pageObject->connection->exec("update ".$pageObject->connection->addTableWrappers("usuarios")
...
$layout = new TLayout("remind_success2", "CityAvenue", "MobileAvenue");
$layout->version = 2;
$layout->blocks["top"] = array();
...
$layout->skinsparams["3"] = array("button"=>"button1");
				...
				$message.= "Nome do Usuário".": ".$data[$cUserNameField]."\r\n";
				$message.= "Senha".": ".$password."\r\n";
				$sentMailResults = runner_mail(array('to' => $data[$cEmailField], 'subject' => "Lembrete de Senha", 'body' => $message));
